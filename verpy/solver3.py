...
class Book:
    ...
    def add_constraint(self, constraint):
        ...
        for package_name in constraint.get_target_package_names():
            if package_name not in self.frontiers:
                self.add_frontier(package_name)

    def get_root_incompatibility(self):
        for constraint in self.constraints:
            if (isinstance(constraint, IncompatibilityConstraint) 
                and len(constraint.get_assignments()) == 1 
                and isinstance(list(constraint.get_assignments())[0], RootAssignment)):
    ...
    def get_incompatibility_constraints_for_package(self, package_name):
        constraints_for_package = []

        for constraint in self.constraints:
            if isinstance(constraint, IncompatibilityConstraint) and constraint.is_constraint_on_package(package_name):
                constraints_for_package.append(constraint)
...
def solve_dependencies(root_dependencies, package_repository):
    ...
    while book.has_frontiers():
        ...
        if package_name == "__root__":
            constraint = book.get_root_incompatibility()
        ...
        allowed_versions = _filter_allowed_version(book.assignments, constraints, package_name, all_versions)
        ...
        if len(allowed_versions) == 0:
            ...
            incompatibilities = _find_incompatibilities(book.assignments, constraints, package_name, all_versions)

            for incompatibilty in incompatibilities:
                if incompatibilty not in book.constraints:
                    logger.debug(f"\t\tFound incompatibility: {incompatibilty}")
                    book.add_constraint(incompatibilty)
        ...
        if book.has_assignment_for_package(package_name):
            ...
            if assignment.version not in allowed_versions:
                ...
                incompatibilities = _find_incompatibilities(book.assignments, constraints, package_name, [assignment.version])

                for incompatibilty in incompatibilities:
                    incompatibilty.assignments.add(assignment)
                    if incompatibilty not in book.constraints:
                        logger.debug(f"\t\tFound incompatibility: {incompatibilty}")
                        book.add_constraint(incompatibilty)
...
def _find_incompatibilities(all_assignments, constraints, package_name, all_versions):
    ...
    for constraint_combination in all_combinations:
        allowed_versions = _filter_allowed_version(all_assignments, constraint_combination, package_name, all_versions)
        if len(allowed_versions) == 0:
            incompatible_assignments = []
            for constraint in constraint_combination:
                incompatible_assignments.extend(constraint.get_assignments())

            # logger.debug(package_name)
            # logger.debug(incompatible_assignments)
            # logger.debug(all_assignments)
            incompatible_assignments = list(filter(lambda x: x.package_name != package_name and x in all_assignments, incompatible_assignments))
            if len(incompatible_assignments) > 0:
                incompatibility = IncompatibilityConstraint(incompatible_assignments, constraint_combination)
